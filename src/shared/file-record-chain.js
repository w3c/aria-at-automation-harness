...
import { host as defaultHost, isDirectory, isFile } from './file-record.js';
...
class FileRecordChain {
   ...
   */
  clone() {
    return new FileRecordChain(this._host.clone(this._record), this._host);
  }
  ...
  static async read(root, options, host = defaultHost) {
    return new FileRecordChain(await host.read(root, options), host);
  }
}
...
class FileRecordChangesChain {
  ...
  describe() {
    return this._host.describeChanges(this._changes);
  }
}
